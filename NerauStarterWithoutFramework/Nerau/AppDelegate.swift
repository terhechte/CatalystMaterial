...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func startTraining(configuration: TrainConfiguration?) {
        ...
        guard let controller = (window?.rootViewController?.storyboard?.instantiateViewController(withIdentifier: "ResultController") as? TrainingViewController) else { fatalError() }
        
        controller.modalPresentationStyle = .formSheet
        (window?.rootViewController as? UITabBarController)?.viewControllers?[0].present(controller, animated: true, completion: {
            controller.doBeginTraining(sender: nil)
        })
    }
}
